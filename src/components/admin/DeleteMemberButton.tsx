...
// Compact variant (list row) just asks a window.confirm.
// Full variant (member detail "danger zone") requires typing DELETE.
    ...
    return (
      <button
        ...
        onClick={(e) => {
          e.preventDefault()
          e.stopPropagation()
          if (window.confirm('Delete this member and all associated records? This cannot be undone.')) {
            void doDelete()
          }
        }}
        disabled={deleting}
    ...
    <div className="border border-red-500/30 bg-red-500/5 rounded-card p-6 space-y-4">
      <div>
        ...
        <p className="font-body text-sm text-white/60 mt-1">
          This permanently deletes the member and all associated records — assessment, rebuild plan, resume
          history, daily encouragements, tool history. This cannot be undone.
        </p>
      </div>
